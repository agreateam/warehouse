        ...
        <div class="container">
            <div class="row">
                <div class="col-4 offset-4" style="margin-top: 10%">
                     ...
                     <small style="margin-left:240px">没有账号？<a href="./register.php">去注册</a></small>
                </div>
            </div>
...
{   
    ...
    if($s['0']=='s')//选择学生
    {
		...
		$uname=$dbh->query("select s_name from s_info where s_id='$usr' and s_key='$pwd';");
        $peo1="select s_name from s_info where s_id='$usr' and s_key='$pwd'";
        
        $row1=$cusr->fetch(PDO::FETCH_BOTH);
        $row2=$cpwd->fetch(PDO::FETCH_BOTH);
		$row3=$uname->fetch(PDO::FETCH_BOTH);
        $result=$dbh->prepare($peo1);
...
<?php
    ...
    else//选择老师
    {
        ...
        $peo2="select t_name from t_info where t_id='$usr'";
		$tname=$dbh->query("select t_name from t_info where t_id='$usr' and t_key='$pwd';");
        $row11=$cusr1->fetch(PDO::FETCH_BOTH);
        $row22=$cpwd1->fetch(PDO::FETCH_BOTH);
		$row33=$tname->fetch(PDO::FETCH_BOTH);
        $result1=$dbh->prepare($peo2);
        if(empty($row11[0]))
                {
                    $dbh=null;
        ?>
